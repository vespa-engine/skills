...
The canonical Vespa migration guide describes a five-phase flow. Fetch the latest version via `https://docs.vespa.ai/en/learn/migrating-from-elastic-search.html.md` (note the `.md` suffix — see [Fetching fresh docs](#fetching-fresh-docs-llm-friendly-resources)).

| Phase | Goal | Tool / Skill |
|---|---|---|
| 1. Export | Dump ES index mapping + documents to disk | `elasticdump` (npm) — exports JSON for mapping and data |
...
| 5. Feed + verify | Reformat ES docs to Vespa feed JSON, load them, validate queries and ranking | `feed-operations` skill + `query-builder` skill |
...
For the export step, see `docs/parser-and-tooling.md` for ElasticDump invocation and the parser's known limitations.
...
| `_doc` / type | document declaration inside the schema |
...
| bulk API (`_bulk`) | `vespa feed` CLI or `/document/v1/` HTTP API |
| reindex API | redeploy schema + visit-and-refeed (or use Vespa's reindexing) |
...
1. **Do not expect bit-for-bit score parity.** Aim for *result-set* parity first (same top-K documents for representative queries), then rank-correlation parity (Spearman / NDCG), then absolute score parity only if a downstream system depends on the score values.
...
2. **Document IDs are hierarchical, not flat.** `_id: "abc123"` becomes `id:mynamespace:mydoctype::abc123`. The namespace is a tenant-level grouping; the doc-type segment must match the schema name. Picking namespaces *before* feeding is much easier than rewriting IDs afterward.
...
6. **Bulk indexing is faster in Vespa — but throttled differently.** ES `_bulk` rejects requests when nodes are saturated; Vespa's `vespa feed` client manages backpressure for you via async HTTP/2. Tune `connections` and `max-streams-per-connection` instead of batch size. See the `feed-operations` skill.

7. **Aliases don't exist.** ES alias-based blue/green index swaps map onto Vespa's *application package versioning* (redeploy with a new schema and let convergence handle the cutover) or onto multi-schema queries (`select … from sources schema_v1, schema_v2 where …`).

8. **Date math, scripts, and runtime fields don't translate 1:1.** ES `"now-1d"`, painless filters, and runtime fields each need a Vespa-side analog: relative dates → use the container's `current_time` or compute in the client; painless filters → ranking expressions or document selections; runtime fields → derived fields in the indexing pipeline.
...
4. Pick one representative query from their current ES workload and translate it to YQL — defer to `query-translation.md` and `query-builder`.
5. Show how to bulk-feed a sample of documents to validate the schema before committing to a full re-index — defer to `feed-operations`.

Resist the urge to do everything in one turn. Each phase has its own gotchas that are easier to surface in isolation.

---

> **For deeper detail**, load `docs/concept-mapping.md` (field-by-field mapping), `docs/query-translation.md` (Query DSL → YQL), `docs/parser-and-tooling.md` (ElasticDump + `ES_Vespa_parser.py`), or `docs/fetching-docs.md` (LLM-friendly doc-fetching patterns) from this skill's directory as needed.
>
